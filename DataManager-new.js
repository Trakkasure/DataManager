...
class DataManager {
    constructor() {
        ...
        this._total = 0;
        this.NONE =  function(){return undefined};
        this.events = ['dataSet','dataChanged','schemaChanged','filterChanged','dimensionCreated','groupCreated']
    ...
    set schema(_schema) {
        ...
        for (var f in _schema) {
            if (funcType === typeof _schema[f])
                _schema[f] = {type:_schema[f],this._filter:f,column:f};
        ...
        this.events.schemaChanged.emit(oldSchema,_schema);
        return this;
    }
    setData(data, clearFilters) {
        ...
        for (f in this.filterFields) {
            ...
            if (clearFilters)
                this.filterFields[f].data = this.NONE;
        ...
        for (f in this.filterFields) {
            if (this.filterFields[f].data!=self.NONE) {
        ...
        if (data.length) {
            if (!this.filterFields[field].dimension) {
                try {
                    ...
                    if (data[0]===this.NONE)
                        this.filterFields[field].data = this.NONE;
                    else
